dojo.require("esri.arcgis.utils",  "esri.layers.graphics", "esri.layers.FeatureLayer")//, "esri.dijit.BasemapGallery");
...
// SRCHECKDAMS Update
//////////////////////////////////////
var SRCHECKDAMS_URL = "https://apps.tucson.ars.ag.gov/srcheckdams/";
var FIELDNAME_IMAGEURL = ["F2017_Pic"]; // photo thumbnail path
var FIELDNAME_DESC1 = ["F2017_Note"]; //text for photo thumbnail

//////////////////////////////////////

var FIELDNAME_NUMBER = ["Number"];
var FIELDNAME_TITLE = ["Location"]
var FIELDNAME_SHORTDESC = ["Desc0"];
...
function init() {
    ...
    // TODO: Remove the message below when the about tab is enabled
    //var current_tab = $.grep($(".tab"), function(n,i){return $(n).text() == "About";});
	...
	// SRCHECKDAMS about info window
	$(".aboutWindowLink").click(function(e) {
        showAbout();
    });
...
function initMap(layers) {
	...
	if (_contentLayers.length > 1) {
         ...
         // The about tab
        $("#tabs").append('<div class="tab" tabindex="0" onclick="activateLayerAbout(_contentLayers[0]), hideBookmarks()">About</div>', true);
        //var introList = $('<li class="mobileTitleThemes" onclick="selectMobileTheme(1)">').append('<span style="margin-left: 30px; margin-right: 30px; vertical-align: middle; line-height: 20px; display: inline-block;">About</span>');
 ...
 ******************************************************/
    ...
    var tab = $.grep($(".tab"), function(n,i){return $(n).text() == "About";})[0];
    tab.click();
	...
	return layerDefinition = {
      ...
      "drawingInfo": {
        "renderer": {
          ...
          "symbol": {
            "type": "esriPMS",
            "url": "https://static.arcgis.com/images/Symbols/Shapes/GreenDiamondLargeB.png",
            "width": 15,
            "height": 15
          }
        }
...
/* This function was added to show the first tab and keep the dam locations active in the map*/
function activateLayerAbout(layer) {
	preSelection();
	_selected = null;
	postSelection();
	_layerCurrent = layer;
    ...
    var aboutString = 'Rock check dams are used throughout the world to mitigate erosion problems on degraded lands. Increasingly, they are being used in restoration efforts on rangelands in the southwestern US; however, their impact on watershed response and channel morphology is not well quantified. In 2008, 37 porous rock structures were built on two small instrumented watersheds on an alluvial fan at the base of the Santa Rita Mountains in southern Arizona, USA. Each watershed is equipped with a rain gauge, supercritical flow flume, and sediment sampler and thirty-five years of historical rainfall and runoff, and sediment data are available to compare with data collected after check dam construction. In addition, post-construction measurements of channel geometry and longitudinal channel profiles are being compared with pre-construction measurements to characterize the impact of check dams on sediment retention and channel morphology. Ultimately vegetation will mitigate soil erosion, and photo sequences from 2009 through 2016 document changes in vegetation in response to the check dams.<br/><br/>' +
...
//////////////////////////////////////////
// SRCHECKDAMS - show 2012 image if 2016 is missing and show 2010 image if 2012 is missing
//////////////////////////////////////////
		//console.log(value.attributes.getValueCI(["F2016_Pic"]));
		if (value.attributes.getValueCI(["F2016_Pic"]) == null){
			FIELDNAME_IMAGEURL = ["F2012_Pic"];
			if (value.attributes.getValueCI(["F2012_Pic"]) == null){
				FIELDNAME_IMAGEURL = ["F2010_Pic"];
			}
		}

		img = $('<img src="' + SRCHECKDAMS_URL + value.attributes.getValueCI(FIELDNAME_IMAGEURL)+'"alt>');
		mobileImg = $('<div style="height: 75px; margin-bottom: 8px;"><img src="' + SRCHECKDAMS_URL + value.attributes.getValueCI(FIELDNAME_IMAGEURL)+'"></div>');
///////////////////////////////////////////
...
function buildPopup(feature, geometry, baseLayerClick)
{
	...
	var picture = atts.getValueCI(["F2009_Pic"]);
...
//////////////////////////////////////////
// SRCHECKDAMS Update - new images
//////////////////////////////////////////
				// 2017 -- add each new year, change dates
				var contDiv = $("<div style='border:1px solid #C7C7C7;text-align: center;padding: 2px;margin-bottom: 5px;'></div>")
				var picture2017 = atts.getValueCI(["F2017_Pic"]);
				var description2017 = atts.getValueCI(["F2017_Note"]);
				$(contDiv).append($(new Image()).attr("src", picture2017));
				$(contDiv).append("<span style='color:#444444;font-weight:bold;'>" + description2017 + "</span>");
				$(pDiv).append(contDiv);
				// 2016
				var contDiv = $("<div style='border:1px solid #C7C7C7;text-align: center;padding: 2px;margin-bottom: 5px;'></div>")
				var picture2016 = atts.getValueCI(["F2016_Pic"]);
				var description2016 = atts.getValueCI(["F2016_Note"]);
				if (picture2016 == null){
					$(contDiv).append("<span style='color:#444444;font-weight:bold;'>No picture available for 2016.</span><br/>");
				}
				else{
					$(contDiv).append($(new Image()).attr("src", picture2016));
					$(contDiv).append("<span style='color:#444444;font-weight:bold;'>" + description2016 + "</span>");
				}
				$(pDiv).append(contDiv);

				// 2012
				var contDiv = $("<div style='border:1px solid #C7C7C7;text-align: center;padding: 2px;margin-bottom: 5px;'></div>")
				var picture2012 = atts.getValueCI(["F2012_Pic"]);
				var description2012 = atts.getValueCI(["F2012_Note"]);
				
				if (picture2012 == null){
					$(contDiv).append("<span style='color:#444444;font-weight:bold;'>No picture available for 2012.</span><br/>");
				}
				else{
					$(contDiv).append($(new Image()).attr("src", picture2012));
					$(contDiv).append("<span style='color:#444444;font-weight:bold;'>" + description2012 + "</span>");
				}
				$(pDiv).append(contDiv);

				// 2010
				var contDiv = $("<div style='border:1px solid #C7C7C7;text-align: center;padding: 2px;margin-bottom: 5px;'></div>")
				var picture2010 = atts.getValueCI(["F2010_Pic"]);
				var description2010 = atts.getValueCI(["F2010_Note"]);
				$(contDiv).append($(new Image()).attr("src", picture2010));
				$(contDiv).append("<span style='color:#444444;font-weight:bold;'>" + description2010 + "</span>");
				$(pDiv).append(contDiv);

				// 2009
				var contDiv = $("<div style='border:1px solid #C7C7C7;text-align: center;padding: 2px;margin-bottom: 5px;'></div>")
				var picture2009 = atts.getValueCI(["F2009_Pic"]);
				var description2009 = atts.getValueCI(["F2009_Note"]);
				$(contDiv).append($(new Image()).attr("src", picture2009));
				$(contDiv).append("<span style='color:#444444;font-weight:bold;'>" + description2009 + "</span>");
				$(pDiv).append(contDiv);
//////////////////////////////////////////
	...
	if (!DETAILS_PANEL) {
		...
		if (website) {
			$(contentDiv).append($('<div class="address"><a href="'+website+'" target="_blank">WebsiteTEST</a></div>').css("padding-top", 10));
			if(baseLayerClick && mobile)
...
// SRCHECKDAMS Update - new images
//////////////////////////////////////////
			// 2017 -- edit for each year, mobile
			var contDiv = $("<div style='border:1px solid #C7C7C7;text-align: center;padding: 2px;margin-bottom: 5px;'></div>")
			var picture2017 = atts.getValueCI(["F2017_Pic"]);
			var description2017 = atts.getValueCI(["F2017_Note"]);
			$(contDiv).append($(new Image()).attr("src", picture2017));
			$(contDiv).append("<span style='color:#444444;font-weight:bold;'>" + description2017 + "</span>");
			$(mobilePDiv).append(contDiv);
			// 2016
			var contDiv = $("<div style='border:1px solid #C7C7C7;text-align: center;padding: 2px;margin-bottom: 5px;'></div>")
			var picture2016 = atts.getValueCI(["F2016_Pic"]);
			var description2016 = atts.getValueCI(["F2016_Note"]);
...
/* function showAbout() {
	...
	var mainDiv = $('<div class="details"></div>');
	var titleDiv = $('<div class="detailsTitle">About</div>');
	var leftDiv = $('<div style="width:100%;">Rock check dams are used throughout the world to mitigate erosion problems on degraded lands. Increasingly, they are being used in restoration efforts on rangelands in the southwestern US; however, their impact on watershed response and channel morphology is not well quantified. In 2008, 37 porous rock structures were built on two small instrumented watersheds on an alluvial fan at the base of the Santa Rita Mountains in southern Arizona, USA. Each watershed is equipped with a rain gauge, supercritical flow flume, and sediment sampler and thirty-five years of historical rainfall and runoff, and sediment data are available to compare with data collected after check dam construction. In addition, post-construction measurements of channel geometry and longitudinal channel profiles are being compared with pre-construction measurements to characterize the impact of check dams on sediment retention and channel morphology. Ultimately vegetation will mitigate soil erosion, and photo sequences from 2009 through 2016 document changes in vegetation in response to the check dams.<br/><br/><a href="http://www.tucson.ars.ag.gov/unit/publications/PDFfiles/2202.pdf" target="_blank">Effect of check dams on runoff, sediment yield and retention on small semi-arid watersheds.</a><br/><br/><a href="http://www.tucson.ars.ag.gov/unit/publications/PDFfiles/2316.pdf" target="_blank">Semiarid watershed response to low-tech porous rock check dams.</a><br/><br/><a href="http://www.tucson.ars.ag.gov/unit/publications/PDFfiles/2513.pdf" target="_blank">The impacts of porous rock check dams on a semiarid alluvial fan.</a></div>');
	...
	$.colorbox({
		...
		maxWidth:"575px",
		scrolling:false,
		onClosed:function(){_showingDetails = false;}
	});
} */
